...
| 假 WASM 运行（固定输出/固定编译错误） | 前端 `VITE_FAKE_WASM=1` | **第 6 天已替换**：真实 clang.wasm 链路落地（Node 自测 6/6 PASS），`VITE_FAKE_WASM` 默认 0；**第 8 天浏览器端实测通过**（playwright 走 登录→运行 A+B→输出 3，见 `docs/images/day8-*.png`） |
...
- [x] **浏览器端真实运行实测**（第 8 天完成）：Vite dev + 题目页点运行 A+B→3（playwright 实测，截图 `docs/images/day8-*.png`）；exnref 不支持时需补 clang22-noeh/lld22-noeh 自托管（当前 21 题均不触发，留作兼容性优化）；
- [ ] **交互器双实现一致性**：前端 JS 交互器 vs 正式 C++ 交互器，逻辑一致性方案——第 9 天前确认；
- [ ] **卡死回收与频率限制参数**：900s 回收已按默认落地；5s 提交间隔后端已实现（42901），前端节流提示待补（G6）；
- [x] 提交状态枚举完整清单与前端徽章配色映射——第 4 天已定稿（全称枚举 + statusMeta 映射）。
- [ ] **题库扩展**：第 8 天已到 21 道（1001~1021，覆盖七类基础题）；G5 细粒度权限与管理端、G7 前端测试、G8 favicon 仍未做（第 9 天）。

## 5. 失效/过时记录（负责人填写）

- 2026-08-24：`FAKE_JUDGE`/`VITE_FAKE_WASM` 默认值由开（true/1）改为关（false/0），对应假实现条目见 §3 已替换状态。
- 2026-08-24：`verify_problems.py` 已支持 21 道题全库自动遍历（第 8 天改造，未实跑），作为第 9 天结课全量回归入口。
