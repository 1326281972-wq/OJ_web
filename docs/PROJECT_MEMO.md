...
| 假实现 | 开关 | 替换日期 |
|--------|------|----------|
| 假评测器（服务端自动判 AC/WA） | 后端 `FAKE_JUDGE=true` | **第 6 天** → 真实 judge_daemon.py（原定第 4 天，因第 4 天改为接口补齐+框架调试顺延，见 gap_list G1） |
| 假 WASM 运行（固定输出/固定编译错误） | 前端 `VITE_FAKE_WASM=1` | **第 6 天** → 真实 clang.wasm（原定第 5 天，因第 5 天聚焦主路径贯通顺延，见 gap_list G2） |

规则：假实现仅用于演示与联调，不改变接口契约；到点后开关默认关闭。

## 4. 待解决问题列表

- [ ] **WASM 资源来源**：采用预编译 clang.wasm（优先），是否自建交叉编译工具链——第 6 天（随 G2 一起定稿）；
